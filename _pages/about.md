...
Meta-SecAlign-70B is the first open-source commercial-grade LLM with built-in prompt injection robustness, which outperforms existing industry solutions from OpenAI/Google. We show that, surprisingly, training on 19K simple instruction-following dataset leads to substantial security to various downstream tasks, especially on agentic workflows such as tool-calling and web-navigation, with attack success rates <2%. We also open-source our training recipe, an improved SecAlign with two modifications to maintain utility, along with evaluation code on 9 utility benchmarks and 6 security benchmarks.
