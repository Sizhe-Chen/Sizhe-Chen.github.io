...
Meta-SecAlign-70B is the first open-source commercial-grade LLM with built-in prompt injection robustness, outperforming gpt-4o and gemini-2.5-flash. We show that, surprisingly, training on 19K instruction-following samples leads to substantial security, which generalizes to even agentic workflows (tool-calling and web-navigation) with attack success rates <2%. We open-source our training recipe, an improved SecAlign with two modifications to maintain utility, along with the evaluation code on 9 utility and 6 security benchmarks.
