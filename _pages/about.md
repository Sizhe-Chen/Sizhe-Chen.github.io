...
Meta-SecAlign-70B is the first open-source commercial-grade LLM with built-in prompt injection robustness outperforming gpt-4o and gemini-2.5-flash. We show that, surprisingly, training on 19K simple instruction-following samples leads to substantial security to various downstream tasks, especially on agentic workflows such as tool-calling and web-navigation with attack success rates <2%. We open-source our training recipe, an improved SecAlign with two modifications to maintain utility, along with evaluation code on 9 utility benchmarks and 6 security benchmarks.
...
DefensiveToken is a test-time defense with prompt injection robustness comparable to training-time alternatives, allowing a flexible switch between the SOTA utility and almost-SOTA security at test time. DefensiveTokens are newly inserted as special tokens, whose embeddings are optimized for security. In security-sensitive cases, system developers can append a few DefensiveTokens. Otherwise, developers can skip DefensiveTokens for a system exactly the same as without defense.
